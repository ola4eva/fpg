...
class payroll_reg(models.TransientModel):
    ...
    # rules = None
    __slots__ = ['rules', 'rules_data', 'mnths_total', 'mnths']
    # mnths = []
    # mnths_total = []
    # rules = []
    # rules_data = []
    ...
    def print_report(self):
        ...
        if datas["form"].get("xls_output", False):
            ...
            sheet.write(
                2, 5, fields.Date.to_string(datas["form"]["start_date"]), title_style1
            )
            sheet.write(2, 6, "To", title_style1)
            sheet.write(
                2, 7, fields.Date.to_string(datas["form"]["end_date"]), title_style1
            )
            main_header = self.get_salary_rules(datas["form"])

            report_header = ["Name", "Job Position", "Department"] + main_header[0]
    ...
    def get_total(self):
        for count in range(1, len(self.mnths_total)):
            if not isinstance(self.mnths_total[count], float) and not isinstance(
                self.mnths_total[count], int
            ):
                continue

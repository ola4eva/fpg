...
class PayrollAdjustment(models.Model):
    ...
    code = fields.Char(
        related="adjustment_type_id.code",
        store=True,
        states={"new": [("readonly", False)]},
    )
    adjustment_line_ids = fields.One2many(
        "adjustment.type.line",
        "adjustment_line_id",
        string="Adjustment",
        readonly=False,
        states={"new": [("readonly", False)]},
    ...
    period_type = fields.Selection(
        [("day", "days"), ("month", "month"), ("year", "year")],
        "Period Type",
        required=True,
        default="month",
        readonly=True,
        ...
        for sub in self:
            for line in sub.adjustment_line_ids:
                if not line.payslip_id.id:
                    ...
                    raise Warning(
                        _(
                            "You can not remove lines now since one payslip has been created. You may close this adjustment and create new adjustment for remaining periods."
                        )
                    )
        if toremove:
...
class AdjustmentTypeLine(models.Model):
    ...
    applied = fields.Boolean(
        string="Applied Payroll ?", default=False, compute="_get_applied", store=True
    )
    payslip_id = fields.Many2one("hr.payslip", string="Payslip", readonly=True)
    amount = fields.Float(
        "Amount",
        help="Enter amount here which should be used to give allowance or deduction to employee for this adjutment line.",
    )

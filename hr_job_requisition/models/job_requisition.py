...
class JobRequisition(models.Model):
    ...
    length_of_ervice = fields.Char('Anticipated Length of Service (for temporary hires)')
    ...
    other_expenses = fields.Float(string="Other anticipated expenses in filling this position (costs of hiring, training, new equipment,")
    # H. Type of Action:
    ...
    exit_reason = fields.Char(string='Exiting Reason')  
    ...
    impact_left_vacant = fields.Char(string="What is the impact on the company if this position is left vacant?")
    ...
    revenue_impact = fields.Char('revenue_impact')
    customer_service_impact = fields.Char('Customer Service Impact')
    other_functions_impact = fields.Char('Other Functions Impact')
    
    # I. List any other information that might be helpful in the recruitment and hiring for this position.
    
    # J. Approvals
    user_id = fields.Many2one('comodel_name', string='user')
    date_submitted = fields.Date(string="Date of Submission")
    hiring_manager_job_id = fields.Many2one('hr.job', string='Position')
    approver_id = fields.Many2one('comodel_name', string='approver')
    date_submitted = fields.Date(string="Date of Submission")
    supervisor_job_id = fields.Many2one('hr.job', string='Position')
    hr_approver_id = fields.Many2one('comodel_name', string='HR')
